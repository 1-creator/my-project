...
class MemoriesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create memory" do
    assert_difference('Memory.count') do
      post memories_url, params: { memory: { description: @memory.description, name: @memory.name, picture: @memory.picture } }
    end
  ...
  test "should update memory" do
    patch memory_url(@memory), params: { memory: { description: @memory.description, name: @memory.name, picture: @memory.picture } }
